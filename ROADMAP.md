...
- [x] 主窗口改造为"**精简模式**"：默认只显示 采集/回放/帧表格/实时信号 4 块。（落地为 5 个核心页：帧/信号/绘图/AI 引擎/DBC 生成器，覆盖"采集→帧表格→出 DBC"工作流；采集/回放入口本在工具栏）
- [x] 其余进阶标签页收进"**高级模式**"折叠菜单，默认隐藏但不删除功能。（视图菜单互斥切换，QSettings 持久化，默认精简；10 处硬编码 tab 索引改为 `_show_tab()`，跳转隐藏页自动升级高级模式，功能零丢失）
- [x] 每个保留 tab 做"默认即可用"：常用操作首屏，深层选项收纳。（dbc_builder 6 个高级导入/导出按钮收纳进「高级导入▾」「高级导出▾」两个弹出菜单；其余核心 tab 常用操作本在首屏）
